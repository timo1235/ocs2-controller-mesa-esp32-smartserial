...
void sserialLoop(void *pvParameters) {
    for (;;) {
        ...
        if (available >= 1) {
            ...
            if (lbp.ct == CT_LOCAL /*11b*/ && lbp.wr == 0) {   // local read, cmd+crc = 2b
                ...
                if (lbp.byte == UnitNumberRPC && available >= 2) {   // unit number, cmd+crc = 2b
                    ...
                    // data_in.joy_x = map(adcManager.getJoystickX(), 0, 26000, 0, 255);
                    // data_in.joy_y = map(adcManager.getJoystickY(), 0, 26000, 0, 255);
                    // data_in.joy_z = map(adcManager.getJoystickZ(), 0, 26000, 0, 255);
        ...
        if (millis() - lastSerialCommunication > 5000 && !timeoutFlag) {
