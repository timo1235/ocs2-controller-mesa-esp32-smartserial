...
uint8_t sserial_slave[] = {
    ...
    0x7E, 0x1E, 0x00, 0x00, 0xCE, 0x00, 0x00, 0x00,   // 240..247
...
void sserialLoop(void *pvParameters) {
    for (;;) {
        ...
        if (available >= 1) {
            ...
            debug.addPrint("Available: %lu, byte: 0x%02X lbp: ct: %d, wr: %d, ai: %d, as: %d, ds: %d, rid: %d, rpc: %d, dummy: %d",
            ...
            if (lbp.ct == CT_LOCAL /*11b*/ && lbp.wr == 0) {   // local read, cmd+crc = 2b
                ...
                if (lbp.byte == UnitNumberRPC && available >= 2) {   // unit number, cmd+crc = 2b
                    ...
                    data_in.joy_x = map(adcManager.getJoystickX(), 0, 26000, -127, 127);
                    data_in.joy_y = map(adcManager.getJoystickY(), 0, 26000, -127, 127);
                    data_in.joy_z = map(adcManager.getJoystickZ(), 0, 26000, -127, 127);
