...
void SSerial::loop() {
    ...
    if (available >= 1) {
        ...
        case CT_RW:
            if (lbp.wr == 0) {
                handleReadWrite(true);
            } else {
                handleReadWrite(false);
            }
            break;
        default:
...
void SSerial::handleRPC() {
    if (lbp.byte == UnitNumberRPC && available >= 2) {   // unit number, cmd+crc = 2b
        ...
        // Let the inRegister and AdcManager set the inputs
        // data_in.in_0 = 1;
...
void SSerial::handleReadWrite(bool isRead) {
    if (isRead) {
        // size = 1 + 2*lbp.as  + 1
        int size = 2 * lbp.as + 2;
        // timeout  = 0;
        ...
        // Dont know what the function of these is, but scale adress has something to do with the global var
...
void SSerial::send(uint8_t len, uint8_t docrc) {
    // Implement the send function logic
}

void SSerial::emptySerialBuffer() {
    // Implement logic to empty the serial buffer
}
