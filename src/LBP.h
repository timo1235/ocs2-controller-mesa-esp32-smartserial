...
struct LBP_Command_Fields_ReadWrite {   // LSB to MSB within bytes, MSB to LSB for bytes themselves
...
enum LBP_Masks : uint8_t { LBP_MASK_COMMAND = 0xC0 };

enum LBP_Constants : uint8_t { LBP_COOKIE = 0x5A };
...
enum LBP_PDD_Data_Types : uint8_t {
    ...
    LBP_PDD_DATA_TYPE_NONVOL_FLOAT    = 0x11,
};
...
enum LBP_PDD_Mode_Types : uint8_t { LBP_PDD_MODE_TYPE_HWMODE = 0x00, LBP_PDD_MODE_TYPE_SWMODE = 0x01 };

struct LBP_PDD_Normal   // size = 64 bytes
{
    LBP_PDD_Record_Types RecordType;
    uint8_t              DataSize;
    uint8_t              DataType;
