...
void tprint(const char *format, ...) {
    ...
    // Fügen Sie die formatierte Zeichenkette zur Warteschlange hinzu
    xQueueSend(queue, &buf, 100);
}
...
void debugSerialTask(void *pvParameters) {
    ...
    char element[256];
    for (;;) {
        // Check if que has elements and process
...
void setup() {
    ...
    Serial1.begin(2500000, SERIAL_8N1, RXD2, TXD2);

    queue = xQueueCreate(100, sizeof(char[256]));
...
// void loop(float period, void *ctx_ptr, hal_pin_inst_t *pin_ptr) {
void loop() { frt_func(); }